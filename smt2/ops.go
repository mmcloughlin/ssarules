...
func buildoperations() map[ast.Opcode]operation {
	...
	var (
		// Unary
		u8 = variant{suffix: "8", arity: 1, signatures: []signature{
			{result: gotypes.Typ[gotypes.Int8], params: unary(gotypes.Int8), dflt: true},
			{result: gotypes.Typ[gotypes.Uint8], params: unary(gotypes.Uint8)},
		}}
		u16 = variant{suffix: "16", arity: 1, signatures: []signature{
			{result: gotypes.Typ[gotypes.Int16], params: unary(gotypes.Int16), dflt: true},
			{result: gotypes.Typ[gotypes.Uint16], params: unary(gotypes.Uint16)},
		}}
		u32 = variant{suffix: "32", arity: 1, signatures: []signature{
			{result: gotypes.Typ[gotypes.Int32], params: unary(gotypes.Int32), dflt: true},
			{result: gotypes.Typ[gotypes.Uint32], params: unary(gotypes.Uint32)},
		}}
		u64 = variant{suffix: "64", arity: 1, signatures: []signature{
			{result: gotypes.Typ[gotypes.Int64], params: unary(gotypes.Int64), dflt: true},
			{result: gotypes.Typ[gotypes.Uint64], params: unary(gotypes.Uint64)},
		}}

		// Binary
		b8 = variant{suffix: "8", arity: 2, signatures: []signature{
			{result: gotypes.Typ[gotypes.Int8], params: binary(gotypes.Int8), dflt: true},
			{result: gotypes.Typ[gotypes.Uint8], params: binary(gotypes.Uint8)},
		}}
		b16 = variant{suffix: "16", arity: 2, signatures: []signature{
			{result: gotypes.Typ[gotypes.Int16], params: binary(gotypes.Int16), dflt: true},
			{result: gotypes.Typ[gotypes.Uint16], params: binary(gotypes.Uint16)},
		}}
		b32 = variant{suffix: "32", arity: 2, signatures: []signature{
			{result: gotypes.Typ[gotypes.Int32], params: binary(gotypes.Int32), dflt: true},
			{result: gotypes.Typ[gotypes.Uint32], params: binary(gotypes.Uint32)},
		}}
		b64 = variant{suffix: "64", arity: 2, signatures: []signature{
			{result: gotypes.Typ[gotypes.Int64], params: binary(gotypes.Int64), dflt: true},
			{result: gotypes.Typ[gotypes.Uint64], params: binary(gotypes.Uint64)},
		}}
		bbool = variant{suffix: "B", arity: 2, signatures: []signature{
			{result: gotypes.Typ[gotypes.Bool], params: binary(gotypes.Bool)},
		}}
	)
	...
	for _, family := range []struct {
		...
		// TODO:	{name: "AddPtr", argLength: 2}, // For address calculations.  arg0 is a pointer and arg1 is an int.
		// TODO:	{name: "Add32F", argLength: 2, commutative: true},
		// TODO:	{name: "Add64F", argLength: 2, commutative: true},
		{"Add", []variant{b8, b16, b32, b64}, "(bvadd %s %s)"},
		// TODO:	{name: "SubPtr", argLength: 2},
		// TODO:	{name: "Sub32F", argLength: 2},
		// TODO:	{name: "Sub64F", argLength: 2},
		{"Sub", []variant{b8, b16, b32, b64}, "(bvsub %s %s)"},

		{"And", []variant{b8, b16, b32, b64}, "(bvand %s %s)"},
		{"Or", []variant{b8, b16, b32, b64}, "(bvor %s %s)"},
		{"Xor", []variant{b8, b16, b32, b64}, "(bvxor %s %s)"},

		// TODO:	{name: "Neg32F", argLength: 1},
		// TODO:	{name: "Neg64F", argLength: 1},
		{"Neg", []variant{u8, u16, u32, u64}, "(bvneg %s)"},
		{"Com", []variant{u8, u16, u32, u64}, "(bvnot %s)"},

		{"And", []variant{bbool}, "(and %s %s)"},
		{"Or", []variant{bbool}, "(or %s %s)"},
		{"Eq", []variant{bbool}, "(= %s %s)"},
		{"Neq", []variant{bbool}, "(not (= %s %s))"},

		{"Not", []variant{{arity: 1, signatures: []signature{{result: gotypes.Typ[gotypes.Bool], params: unary(gotypes.Bool)}}}}, "(not %s)"},
	} {
		for _, v := range family.variants {
			operations[ast.Opcode(family.name+v.suffix)] = &simple{
...
func (s *signature) match(params []gotypes.Type) bool {
	if len(params) != len(s.params) {
		return false
	}
	for i := range s.params {
		if !gotypes.Identical(s.params[i], params[i]) {
			return false
		}
	}
	return true
...
func unary(kind types.BasicKind) []gotypes.Type {
	return []gotypes.Type{gotypes.Typ[kind]}
}

func binary(kind types.BasicKind) []gotypes.Type {
	return []gotypes.Type{gotypes.Typ[kind], gotypes.Typ[kind]}
}
...
func (s *simple) Params(result gotypes.Type) ([]gotypes.Type, error) {
	// If result has a known type, expect a matching signature.
	if result != nil {
		for _, s := range s.signatures {
			if gotypes.Identical(result, s.result) {
				return s.params, nil
			}
		}
		return nil, fmt.Errorf("invalid return type %s", result)
	}

	// Result not known. If there's only one signature, return that.
	if len(s.signatures) == 1 {
		return s.signatures[0].params, nil
	}

	// Or return default if that's defined.
	for _, s := range s.signatures {
		if s.dflt {
			return s.params, nil
		}
	...
	unknown := make([]gotypes.Type, s.arity)
...
func (s *simple) Evaluate(in *input) (*value, error) {
	...
	if result == nil {
		return nil, errors.New("cannot infer return type")
	}
